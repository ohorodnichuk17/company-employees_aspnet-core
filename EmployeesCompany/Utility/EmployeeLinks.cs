...
namespace EmployeesCompany.Utility;
...
public class EmployeeLinks : IEmployeeLinks
{
    ...
    {
        if (httpContext.Items["AcceptHeaderMediaType"] is MediaTypeHeaderValue mediaType)
        {
            var subType = mediaType.MediaType.Split('/').Last();
            return subType.EndsWith("hateoas", StringComparison.InvariantCultureIgnoreCase);
        }
        return false;
    }
